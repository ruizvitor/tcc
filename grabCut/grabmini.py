...
def main():
    ...
    while(1):
        ...
        if k == 27:         # esc to exit
            ...
            # if(resize==1):
            #     print("Doing resizing and shift")
            #
            #     # bgdmodel = np.zeros((1,65),np.float64)
            #     # fgdmodel = np.zeros((1,65),np.float64)
            #     # # mask = cv2.resize(mask, (0,0), fx=8, fy=8)
            #     # mask = np.zeros(img3.shape[:2],dtype = np.uint8)
            #     # rect=(rect[0]*8,rect[1]*8,rect[2]*8,rect[3]*8)
            #     # cv2.grabCut(img3,mask,rect,bgdmodel,fgdmodel,1,cv2.GC_INIT_WITH_RECT)
            #     #
            #     # mask2 = np.where((mask==1) + (mask==3),255,0).astype('uint8')
            #     # output = cv2.bitwise_and(img3,img3,mask=mask2)
            #
            #
